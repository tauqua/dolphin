...
void EnhancementsWidget::CreateWidgets()
{
  ...
  m_configure_custom_shaders = new QPushButton(tr("Configure..."));
  ...
  m_custom_shader_window = new CustomShaderWindow(this);
  enhancements_layout->addWidget(new QLabel(tr("Custom Shaders:")), 4, 0);
  enhancements_layout->addWidget(m_configure_custom_shaders, 4, 1);
...
void EnhancementsWidget::ConnectWidgets()
{
  ...
  connect(m_configure_custom_shaders, &QPushButton::pressed, [this] {
    m_custom_shader_window->show();
    m_custom_shader_window->raise();
    m_custom_shader_window->activateWindow();
  });
}
